...
public class WeatherForecastRequestScopedView {
    ...
    public void init() { // Use this method to initialized fields instead of a constructor
        ...
        for (int count = 1;  count <= 5; count++)
        {
            ...
            // currentWeatherForecast.setCity(faker.address().city());
            // currentWeatherForecast.setDate(LocalDate.now().plusDays(randomGenerator.nextInt(1, 6)));
            // selectedFirebcurrentWeatherForecastaseWeatherForecast.setTemperatureCelsius(faker.number().numberBetween(-20,50));
            // currentWeatherForecast.setDescription(faker.weather().description());
